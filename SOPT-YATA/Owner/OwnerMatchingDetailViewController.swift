...
class OwnerMatchingDetailViewController: UIViewController {
    ...
    @IBAction func callToDriver(_ sender: UIButton) {
        if let url = URL(string: "tel://\(data!.user_phone)"), UIApplication.shared.canOpenURL(url) {
    ...
    @IBAction func confirm(_ sender: UIButton) {
        Network().putMatchingComplete(self.data?.applying_idx ?? 0) { (result) in
            if result {
                self.performSegue(withIdentifier: "toComplete", sender: self)
            }
        }
    }
}
